...
export function GiftModule({ config, theme }) {
  ...
  return (
    <div className="rounded-[2rem] border p-8 shadow-xl" style={{ background: theme.card, borderColor: hexToRgba(theme.primary, 0.14) }}>
      ...
      {isDemo && (
        <p className="mt-5 rounded-2xl px-4 py-3 text-sm font-semibold" style={{ background: theme.background, color: theme.muted }}>{DEMO_DISABLED_MESSAGE} Los datos de regalo son ficticios.</p>
      )}
      {!isDemo && (gift.alias || gift.cbu) && (
